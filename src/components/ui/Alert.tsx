...
export function Alert({ className, state = 'info', showIcon = true, icon, children, ...props }: AlertProps) {
  const isClient = useIsClient();

  if (!isClient) return null;
  return (
    <div role="alert" className={cn(alert({ state, className }))} {...props}>
      <Show if={showIcon}>
        <div className="pr-4">{icon ? <>{icon}</> : <>{defaultIconsMap[state as keyof typeof defaultIconsMap]}</>}</div>
      </Show>
