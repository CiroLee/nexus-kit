...
export default function Accordion({ className, items, ...props }: AccordionProps) {
  return (
    <AccordionPrimitive.Root className={className} {...props}>
      {items.map((item) => (
        <AccordionPrimitive.Item data-slot="accordion-item" key={item.value} value={item.value} disabled={item.disabled} className="border-line border-b data-disabled:opacity-50">
