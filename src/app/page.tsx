...
export default function Home() {
  return (
    <div className="relative px-4 sm:px-8">
      <div className="relative mx-auto mt-[16%] w-[90vw] border border-teal-500/50 p-5 sm:mt-30 sm:max-w-3xl">
        <span className="absolute top-0 left-0 size-2.5 bg-teal-500"></span>
        <span className="absolute top-0 right-0 size-2.5 bg-teal-500"></span>
        <span className="absolute right-0 bottom-0 size-2.5 bg-teal-500"></span>
        <span className="absolute bottom-0 left-0 size-2.5 bg-teal-500"></span>
        <Heading as="h1" className="sm:text-7xl">
          Nexus-Kit
        </Heading>
        <p className="mt-4 text-gray-200">A practical and beautiful component library, built based on radix-ui and tailwind css v4, just copy and paste</p>
      </div>
      ...
      <section>
        ...
        <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:mx-auto xl:max-w-7xl xl:grid-cols-4">
          <ShowCard
            ...
            title={`${navConfig.length}+ Free components`}
            description={`more than ${navConfig.length} practical and beautiful components, they are FREE`}
            className="w-full"
          />
